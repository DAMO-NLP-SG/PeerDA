...
def get_answers_from_example(examples):
    results = {}
    for qa in examples:
        id = qa.qas_id
        answers = qa.answers
...
def compute_precision_recall(gt_dict, preds_dict, category=None, zero_shot=False):
    tp, fp, fn = 0, 0, 0
            ...
            for ans in answers:
                ...
                for pred in preds:
                    if substr_ok:
                        ...
                        if zero_shot and not is_match and pred in ans:
                            is_match = True
                    if is_match:
                        match_found = True
...
def get_precisions_recalls(pred_dict, gt_dict, category=None, zero_shot=False):
    precisions = [1]
    recalls = [0]
    confs = []
    for conf in list(np.arange(0.99, 0, -0.01)) + [0.001, 0]:
        conf_thresh_pred_dict = get_preds(pred_dict, conf)
        prec, recall = compute_precision_recall(gt_dict, conf_thresh_pred_dict, category=category, zero_shot=zero_shot)
...
def get_results(model_path, gt_dict, verbose=False, zero_shot=False):
    name = model_path

    pred_dict = load_json(model_path)

    assert sorted(list(pred_dict.keys())) == sorted(list(gt_dict.keys()))

    precisions, recalls, confs = get_precisions_recalls(pred_dict, gt_dict, zero_shot=zero_shot)
...
if __name__ == "__main__":
    test_json_path = "../Data/CCE/mrc-cce.test"
    model_path = "../saved_models/DA-roberta-base-CCE-expand0.5/nbest_predictions_-10.json"
    save_dir = "./results"
