# Copyright 2018 The HuggingFace Team, and The Alibaba Damo Academy team.
#
...
def contract_convert_example_to_features(
    example, max_seq_length, max_query_length, doc_stride, padding_strategy, is_training, tokenizer
):
    features = []
...
def contract_convert_examples_to_features(
    ...
    for contract_features in tqdm(
        ...
        for example_feature in contract_features:
            ...
            new_contracts_features.append(example_feature)
            unique_id += 1
        example_index += 1
    ...
    if return_dataset == "pt":
        ...
        all_input_ids = torch.tensor([f.input_ids for f in contracts_features], dtype=torch.long)
        all_attention_masks = torch.tensor([f.attention_mask for f in contracts_features], dtype=torch.long)
        all_token_type_ids = torch.tensor([f.token_type_ids for f in contracts_features], dtype=torch.long)
        all_cls_index = torch.tensor([f.cls_index for f in contracts_features], dtype=torch.long)
        all_p_mask = torch.tensor([f.p_mask for f in contracts_features], dtype=torch.float)
        all_is_impossible = torch.tensor([f.is_impossible for f in contracts_features], dtype=torch.float)
        ...
        if not is_training:
            ...
            all_start_positions = torch.tensor([f.start_position for f in contracts_features], dtype=torch.long)
            all_end_positions = torch.tensor([f.end_position for f in contracts_features], dtype=torch.long)
            dataset = TensorDataset(
                all_input_ids,
                all_attention_masks,
                all_token_type_ids,
        ...
        return contracts_features, dataset
    ...
    for contract_examples in tqdm(contracts_examples, desc="convert contract examples to features at worker {}".format(str(i_w))):
        ...
        for contract_example in contract_examples:
            example_features = contract_convert_example_to_features(
                contract_example, max_seq_length, max_query_length, doc_stride, padding_strategy, is_training, tokenizer,
            )
...
class Dataloader(object):
    ...
    def __iter__(self):
        ...
        for batch in batches:
            ...
            #query
            # query_input_ids = torch.tensor([query_feature.input_ids for query_feature in queries_features], dtype=torch.long)
            # query_attention_masks = torch.tensor([query_feature.attention_mask for query_feature in queries_features], dtype=torch.long)
            # query_token_type_ids = torch.tensor([query_feature.token_type_ids for query_feature in queries_features], dtype=torch.long)
            #contract
            max_doc_length = max([len(x) for x in contracts_features])
            contract_input_ids = torch.tensor([[doc.input_ids for doc in contract_features] +
                                               [[self.tokenizer.pad_token_id] * len(contract_features[0].input_ids)
                                                for _ in range(max_doc_length - len(contract_features))] for contract_features in contracts_features], dtype=torch.long)
            contract_attention_mask = torch.tensor([[doc.attention_mask for doc in contract_features] +
                                               [[0] * len(contract_features[0].attention_mask)
                                                for _ in range(max_doc_length - len(contract_features))] for contract_features in contracts_features], dtype=torch.long)
            contract_token_type_ids = torch.tensor([[doc.token_type_ids for doc in contract_features]+
                                               [[0] * len(contract_features[0].token_type_ids)
                                                for _ in range(max_doc_length - len(contract_features))] for contract_features in contracts_features], dtype=torch.long)
            contract_eval_ids = torch.tensor([[doc.eval_id for doc in contract_features] +
                                     [-1 for _ in range(max_doc_length - len(contract_features))] for
            ...
            if self.is_training:
                contract_start_position = torch.tensor([[doc.start_position for doc in contract_features] +
                                               [0 for _ in range(max_doc_length - len(contract_features))] for contract_features in contracts_features], dtype=torch.long)
                ...
                contract_label_ids = torch.tensor([[doc.label_id for doc in contract_features] +
                                                      [self.query_size for _ in range(max_doc_length - len(contract_features))] for
                                                      contract_features in contracts_features], dtype=torch.long)

                yield (contract_input_ids, contract_attention_mask, contract_token_type_ids, contract_start_position, contract_end_position, contract_eval_ids, contract_label_ids)
            else:
                yield (contract_input_ids, contract_attention_mask, contract_token_type_ids, contract_eval_ids)