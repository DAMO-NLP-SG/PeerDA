...
from utils import collate_to_max_length_roberta, collate_to_max_length_bert, query_span_f1
...
def evaluate(args, data_path, model, tokenizer, prefix="", dataset_name="mrc-ner.test"):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        results = query_span_f1(start_preds=start_preds, end_preds=end_preds, match_logits=span_logits,
                                label_mask=batch[5], match_labels=batch[6])
...
def main():
    ...
    parser.add_argument(
        "--cache_data",
        action="store_true",
        help="save cache file for training data",
    )
    parser.add_argument(
        "--is_chinese",
        action="store_true",
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            if args.data_path.endswith('restaurant') or args.data_path.endswith('movie'):
                dev_results = evaluate(args, args.data_path, model, tokenizer, prefix='', dataset_name="mrc-ner.test")
            else:
            ...
            if dev_results[0] > best_dev:
                ...
                model.to(args.device)
            test_results = evaluate(args, args.data_path, model, tokenizer, prefix='best')
            best_test, best_p, best_r = test_results[0], test_results[1], test_results[2]
            ...
            best_test, best_p, best_r = Decimal(best_test.data.item() * 100).quantize(Decimal("0.01"),rounding="ROUND_HALF_UP"), Decimal(
                best_p.data.item() * 100).quantize(Decimal("0.01"), rounding="ROUND_HALF_UP"), Decimal(
                best_r.data.item() * 100).quantize(Decimal("0.01"), rounding="ROUND_HALF_UP")
            ...
            with open('result_' + data_name + '.txt', 'a') as writer:
                save_string = "\t".join(
                    [args.model_name_or_path, args.data_path, str(args.learning_rate), str(args.num_train_epochs),
                     str(args.per_gpu_train_batch_size), str(args.expand_rate), str(args.sizeonly), str(best_dev), str(best_test), str(best_p), str(best_r)])
                writer.write(save_string + '\n')
        else:
            global_step = ""
            # evaluate(args, args.data_path, model, tokenizer, prefix='', dataset_name="mrc-ner.dev")
            if args.fp16:
                try:
                    from apex import amp
                except ImportError:
            ...
            result = evaluate(args, args.data_path, model, tokenizer)
