...
def conll2mrc(conll_f):
    count = 0
    label_details = {'Rating': "An evaluation or assessment of something, in terms of quality, quantity, or some combination of both.",
                     'Amenity': "Something considered to benefit a location, contribute to its enjoyment, and thereby increase its value.",
                     'Location': "Location or place are used to denote a region (point, line, or area) on Earth’s surface or elsewhere.",
                     'Restaurant_Name': "The name of a business that prepares and serves food and drinks to customers.",
                     'Price': "The (usually not negative) quantity of payment or compensation given by one party to another in return for one unit of goods or services.",
                     'Hours': "The continued sequence of existence and events that occurs in an apparently irreversible succession from the past, through the present, into the future.",
                     'Dish': "Something prepared to be eaten.",
...
if __name__ == "__main__":
    for data_type in ['train', 'test']:
        data_file_path = os.path.join("./Data/restaurant", "{}.{}".format("tag-ner", data_type))
        conll_f = read_conll(data_file_path, delimiter="\t")
        mrc_f = conll2mrc(conll_f)
        save_file = os.path.join("./Data/restaurant", "{}.{}".format("mrc-ner", data_type))
