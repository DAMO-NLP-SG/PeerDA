...
from utils import collate_to_max_length_bert, collate_to_max_length_roberta, generate_span, merge_article_annotations
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for i_b, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        if i_b < 122:
            continue
    ...
    all_results_dict = {}
    for one_span in all_results:
        key = one_span[0:2]
        if key not in all_results_dict:
            all_results_dict[key] = []
        all_results_dict[key].append(one_span[2:4])
    for one_query in all_results_dict:
        data_id = one_query[0] * len(label_map) + one_query[1]
        data_one = dataset.all_data[data_id]
        article_id = data_one['article_id']
        char_offset = data_one["char_offset"]
        spans = all_results_dict[one_query]
        spans = sorted(spans, key=lambda x: (x[0], -x[1]))
        # spans = [x for x in spans if x[0] < x[1]]
        while(merge_article_annotations(spans, len(spans) - 1)):
            pass
        for i in reversed(range(len(spans) - 1)):
            merge_article_annotations(spans, i)
        for span in spans:
            span_type = label_map[one_query[1]]
            start = char_offset + span[0]
            end = char_offset + span[1]
            save_line = [article_id, span_type, str(start), str(end)]
            save_line = "\t".join(save_line) + '\n'
            prediction.append(save_line)
    if test:
        pred_save_dir = os.path.join(args.output_dir, "prediction_test.tsv")
    else:
        gold_save_dir = os.path.join(args.data_path, "dev-labels-task-flc-tc")
        pred_save_dir = os.path.join(args.output_dir, "prediction_dev.tsv")
    with open(pred_save_dir, 'w') as writer:
        writer.writelines(prediction)
    if not test:
        os.system(
            '../../../anaconda3/bin/python ./SemEval20/evaluation/task-FLC_scorer.py -s {} -r {} -p ./SemEval20/propaganda-techniques-names-semeval2020task11.txt'.format(pred_save_dir, gold_save_dir))
    return
