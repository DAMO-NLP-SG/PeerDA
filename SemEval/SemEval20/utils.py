...
def merge_article_annotations(spans, i):
    """
    ...
    for j in range(0, i): #len(annotations_without_overlapping)):
        assert i<len(spans) or print(i, len(spans))
        if j != i and span_overlapping(spans[i],spans[j]):
            new_span = (min(spans[i][0], spans[j][0]), max(spans[i][1], spans[j][1]))
            spans[j] = new_span
            del(spans[i])

            if j > i:
                j -= 1
            merge_article_annotations(spans, j)
            # print("done")
            return True
