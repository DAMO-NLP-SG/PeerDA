...
class MRCSEDataset(Dataset):
    def __init__(self, json_path, tokenizer: None, max_length: int = 512,max_query_length = 128, possible_only=False, pad_to_maxlen=False, is_training=False, rate=1, DA=False, is_chinese=False, sizeonly=False):
    ...
    def get_labels(self):
     ...
     'Whataboutism,Straw_Men,Red_Herring':  "A technique that attempts to discredit an opponent’s position by charging them with hypocrisy without directly disproving their argument.",
        ...
        self.label_size = len(self.label_dict)
    ...
    def prompt(self):
        ...
        for i_d, data in enumerate(self.all_data):
            qas_id = data['qas_id']
            context_id = qas_id.split(".")[0]
            label = data['entity_label']
            details = data['query']
                ...
                for ic,c in enumerate(context):
                    if _is_whitespace(c):
                        ...
                        if prev_is_whitespace:
                            words.append(c)
                            word_to_char_offset.append(ic)
                        else:
                            words[-1] += c
                        prev_is_whitespace = False
                    char_to_word_offset.append(len(words) - 1)

            char_start_positions = data["start_position"]
            char_end_positions = data['end_position']
            start_positions = [char_to_word_offset[x] for x in char_start_positions]
            end_positions = [char_to_word_offset[x] for x in char_end_positions]
            if self.is_chinese:
            ...
            negative_start_position = [char_to_word_offset[y] for x in negative_start_position for y in x]
            negative_end_position = [char_to_word_offset[y] for x in negative_end_position for y in x]
            new_data = {
                "article_id": article_id,
                "char_start_positions": char_start_positions,
                "char_end_positions":char_end_positions,
    ...
    def __getitem__(self, item):
        ...
        end_positions = data["end_position"]
        spans = data['spans']
        word_to_char_offset = data['word_to_char_offset']
        ...
        for (i, token) in enumerate(context):
            ...
            for sub_token in sub_tokens:
                textform = sub_token.replace("##", "").replace('âĢ', "").replace("Ġ","") # for character level evaluation only
                tok_to_orig_index.append((i, offset_index))
                all_doc_tokens.append(sub_token)
                offset_index += len(textform)
        ...
        improved_positions = [_improve_answer_span(all_doc_tokens, tok_start_positions[it], tok_end_positions[it], tokenizer, spans[it]) for it in range(len(tok_start_positions))]
        tok_start_positions = [x[0] for x in improved_positions]
        tok_end_positions = [x[1] for x in improved_positions]
        ...
        new_end_positions = [x + doc_offset if (x + doc_offset) < self.max_length - 1 else self.max_length - 2 for x in tok_end_positions ]
        new_end_positions = new_end_positions[:len(new_start_positions)]
        if not self.is_training:
            tok_to_char_index = [-1] * doc_offset + [word_to_char_offset[x[0]] + x[1] for x in tok_to_orig_index]
            data['tok_to_char_index'] = tok_to_char_index
