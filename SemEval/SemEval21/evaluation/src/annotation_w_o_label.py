...
class AnnotationWithOutLabel(object):
    ...
    def get_span_content(self, text:str)->str: 
        """
        Given an article as a string, returns the content of the annotation
        """
        return text[self.get_start_offset():self.get_end_offset()]        
    ...
    def shrink_annotation(self, number_of_chars_removed:int)->None:
        """
        Shrink the annotation by -1*number_of_chars_removed
        """
        self.set_end_offset(self.get_end_offset() + number_of_chars_removed)
    ...
    def span_intersection(self, second_annotation:AnnotationWithOutLabel)->int:
        return len(self.get_span().intersection(second_annotation.get_span()))
