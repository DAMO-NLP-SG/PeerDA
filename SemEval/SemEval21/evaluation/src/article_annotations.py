...
import src.annotation_w_confidence as anwc
...
class Articles_annotations(object):
    ...
    def _matching_annotations(self, second_annotations:Articles_annotations, i:int)->list(int, int):
            """
            Starting from annotation of index i, loops through the annotations with identical span to i
            and returns the indices of the first pair of annotations with identical labels among each others. 
            If there is no such pair, (-1,-1) is returned.  
            """
            j2 = i
    ...
    def adjust_annotations(self, start_index:int, offset:int, original_annotations:AnnotationWithOutLabel,text:str)->None:
        """

        """
        for span, original_span in zip(self.spans, original_annotations.spans):
            if start_index >= original_span.get_start_offset() and start_index < original_span.get_end_offset():
                #print("SHRINK %d - %s"%(start_index, original_span),end="")
                span.shrink_annotation(offset)
                #print(" - %s -%s-"%(span, text[span.get_start_offset():span.get_end_offset()]))
            else:
                if start_index < original_span.get_start_offset():
                    #print("SHIFT %d - %s"%(start_index, original_span), end="")
                    span.shift_annotation(offset)
                    #print(" - %s -%s-"%(span, text[span.get_start_offset():span.get_end_offset()]))


    def align_annotation_to_new_text(self, original_text:str, new_text:str)->Articles_annotations:
        """
        ...
        This function currently assumes that 
            1) new_text has only additional spaces w.r.t original_text
            2) there have been a number of char transformation in new_text (see code below)
        """
        original_annotations = copy.deepcopy(self)
        i,j=(0,0)
        while i<len(original_text) and j<len(new_text):
            ...
            if n==o:
                ...
                #print("***", o,n,i,j,original_text[max(i-5,0):min(i+5,len(original_text))], " --- ", new_text[max(0,j-5):min(j+5,len(new_text))], end="\n")
                if (o=='’' and n=='\'') \
                    or (o=='`' and n=='\'') \
                        or (o=='“' and n=='"') \
                            or (o=='‘' and n=='"') \
                    ...
                    if i+1<len(original_text) and o=='\'' and original_text[i+1]=='\'' and n=='"':
                        i+=2
                        j+=1
                    else:
                        if new_text[j] != " ":
                            print("Failed alignment, unexpected char pairs:%s-%s"%(original_text[i],new_text[j]))
                            print("or.:%s\nnew:%s"%(original_text[max(i-5,0):min(i+5,len(original_text))], new_text[max(0,j-5):min(j+5,len(new_text))]))
                            sys.exit()
                        self.adjust_annotations(j,-1, original_annotations, original_text)
                        j+=1
    ...
    def align_annotations(self, second_annotations:Articles_annotations)->None:
        """
        Reorder all annotations with the same span such that the matching between annotations' labels
        and the ones from second_annotations is maximised. 
        The function is currently called after checking the annotation spans match. TODO: need to add a check that has actually been done
        ...
        while i < len(self):
            ...
            #i+=1 this command had to be moved after the if (thanks to Anton Cherniavskii who pointed it out)
    ...
    def __eq__(self, second_article_annotations:Articles_annotations):
        """
        compare two sets of annotations. Notice that, at the moment, annotations are not sorted. 
        """
        return self.check_article_ids_and_spans(second_article_annotations)
    ...
    def __sub__(self, second_article_annotations:Articles_annotations):

        diffs = []
        for annotation1, annotation2 in zip(self, second_article_annotations):
            if not annotation1==annotation2:
                diffs.append((annotation1, annotation2))
        return diffs


    def check_article_ids_and_spans(self, second_article_annotations:Articles_annotations)->bool:

        if self.get_article_id() != second_article_annotations.get_article_id():
            return False
        for annotation1, annotation2 in zip(self, second_article_annotations):
            if not annotation1==annotation2:
                return False
    ...
    def get_markers_from_spans(self):
        ...
        for i, annotation in enumerate(self.spans, 1):
            self.markers.append((annotation.get_start_offset(), annotation.get_label(), i, "start", annotation.get_confidence()))
            self.markers.append((annotation.get_end_offset(), annotation.get_label(), i, "end", annotation.get_confidence()))
    ...
    def load_article_annotations_from_csv_file(self, filename:str, annotation_class=anwc.AnnotationWithConfidence):
        """
    ...
    def marker_confidence(self, marker_index=None):

        if marker_index is None:
            marker_index = self.curr_marker
        if marker_index < len(self.markers):
            return self.markers[marker_index][4]
    ...
    def start_annotation_marker_function(self, annotations_stack:list, marker_index:int, row_counter:int)->str:
        ...
        return '<span id="row%dannotation%d" class="%s"' \
                %(row_counter, self.marker_annotation(marker_index), " ".join([ "technique%d"%(self.technique_index_from_annotation_index(x)) for x in annotations_stack + [ self.marker_annotation(marker_index) ] ])) + \
                    "".join([ ' technique%d="%f"'%(self.technique_index_from_annotation_index(x), self.marker_confidence(self.annotation_stack_index_to_markers_index(x))) for x in annotations_stack + [ self.marker_annotation(marker_index) ] ]) + ">"
        #return '<span id="row%dannotation%d" class="%s" confidence="%s">' \
        #        %(row_counter, self.marker_annotation(marker_index), " ".join([ "technique%d"%(self.technique_index_from_annotation_index(x)) for x in annotations_stack + [ self.marker_annotation(marker_index) ] ]), " ".join([ str(self.marker_confidence(self.annotation_stack_index_to_markers_index(x))) for x in annotations_stack + [ self.marker_annotation(marker_index) ] ]) ) 
    ...
    def get_spans_content(self, article_content:str)->str: 
        ...
        s = ""
        for annotation in self.get_article_annotations():
            s += annotation.get_span_content(article_content) + "\n"
        return s
    ...
    @classmethod
    def set_output_format_article_id(cls, new_val:bool)->None:

        cls.output_format_article_id = new_val

    @classmethod
    def set_output_format_article_label(cls, new_val:bool)->None:

        cls.output_format_article_label = new_val

    @classmethod
    def set_output_format_article_spans(cls, new_val:bool)->None:

        cls.output_format_article_spans = new_val
    ...
    def save_annotations_to_file(self, filename)->None:
        
        with open(filename, "w") as f:
            f.write(self.annotations_to_string_csv())


    def shift_spans(self, start_index, offset)->None:
