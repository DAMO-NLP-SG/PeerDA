...
def main(args):
    ...
    gold_annotations.load_annotation_list_from_folder(gold_file)
    ...
    if not user_annotations.compare_annotations_article_lists(gold_annotations) or not user_annotations.check_overlapping_annotation_spans(False, False) or not gold_annotations.check_overlapping_annotation_spans(False, True):
        logger.error("wrong format, no scoring will be performed")
        sys.exit()
    logger.info("OK: submission file format appears to be correct")
    res_for_output, res_for_script = user_annotations.FLC_score_to_string(gold_annotations, output_for_script)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("Scorer for Task FLC - PTC corpus.\n" +
    "Example: python3 task-FLC_scorer.py -s data/submission-task-FLC-articles.tsv -r data/articles_FLC_annotations -p data/propaganda-techniques-names-semeval2020task11.txt")

    parser.add_argument('-s', '--submission-file', dest='submission', required=True, help="file with the submission of the team")
    parser.add_argument('-r', '--reference-folder', dest='gold', required=True, help="folder with the gold label files.")
