...
class MRCNERDataset(Dataset):
    def __init__(self, json_path, tokenizer: None, max_length: int = 512, max_query_length = 64, possible_only=False, pad_to_maxlen=False, is_training=False, rate=1, DA=False, is_chinese=False, sizeonly=False, ate=False):
        self.all_data = json.load(open(json_path, encoding="utf-8"))
        self.is_chinese = is_chinese
        self.ate = ate
        self.rate = rate
        self.prompt()
        self.get_category_label()
    ...
    def prompt(self):
        ...
        for data in self.all_data:
            ...
            start_positions = data["start_position"]
            end_positions = data["end_position"]
            category_spans = data['category_span']
            words = context.split()
            ...
            category_spans = [[" ".join(words[start_positions[i]: end_positions[i] + 1]), category_spans[i]] for i in range(len(start_positions))]

            assert len(category_spans) == len(start_positions)
    ...
    def peering_category(self):
        ...
        for ie, example in enumerate(examples):
            ...
            if not example["impossible"]:
                answers = [x[0] for x in example['category_span']]
        ...
        for label_id in label_dict:
            ...
            if gap == 0:
                continue
            possible_item = [x for x in item if x[1] is not None and x[1] != 'NULL']
