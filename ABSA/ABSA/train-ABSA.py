...
from utils import collate_to_max_length_roberta, collate_to_max_length_bert, generate_span, tuple_f1_scores
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    if args.evaluate_during_training:
                        ...
                        test_results = evaluate(args, args.data_path, model, tokenizer, prefix=global_step,
                                                split="test")
                        if test_results['f1'] > best_test:
                            best_test = test_results['f1']
                        if dev_results['f1'] > best_dev:
                            best_dev = dev_results['f1']
...
def evaluate(args, data_path, model, tokenizer, prefix="", split='dev'):
    dataset = load_and_cache_examples(args, tokenizer, evaluate=True, split=split)
    ...
    absa_golds = []
    absa_preds = []
    for i_e in range(len(dataset.all_data) // dataset.label_size):
        gold_pair = []
        pred_pair = []
        predition_span = [y for x in all_results for y in x[1] if y[0] == i_e]
        gold_span = [y for x in all_results for y in x[0] if y[0] == i_e]
        for i_l in range(dataset.label_size):
            example = dataset.all_data[i_e * dataset.label_size + i_l]
            assert "{}.{}".format(i_e, i_l) == example['qas_id']
            sentiment = example['entity_label']
            pred_span_label = [x for x in predition_span if x[1] == i_l]
            gold_span_label = [x for x in gold_span if x[1] == i_l]
            for span in pred_span_label:
                pred_pair.append((sentiment, span[2]))
            for span in gold_span_label:
                gold_pair.append((sentiment, span[2]))
        absa_golds.append(gold_pair)
        absa_preds.append(pred_pair)
    scores_absa = tuple_f1_scores(absa_preds, absa_golds)
    if args.ate:
        logger.warning(
            f"EVAL ATE INFO {data_path} {split} -> valid_f1 is: {scores_absa['f1']}; precision: {scores_absa['precision']}, recall: {scores_absa['recall']}.")
    else:
        logger.warning(f"EVAL UABSA INFO {data_path} {split} -> valid_f1 is: {scores_absa['f1']}; precision: {scores_absa['precision']}, recall: {scores_absa['recall']}.")
...
def load_and_cache_examples(args, tokenizer, evaluate=False, split= 'train'):
    ...
    if evaluate:
        if args.ate:
            json_path = os.path.join(args.data_path, f"mrc-ate." + split)
        else:
            json_path = os.path.join(args.data_path, f"mrc-uabsa." + split)

    else:
        if args.ate:
            json_path = os.path.join(args.data_path, f"mrc-ate.train")
        else:
            json_path = os.path.join(args.data_path, f"mrc-uabsa.train")
...
def main():
    ...
    parser.add_argument(
        ...
        default=0,
        type=float,
        help="expanding method of matching data, 0 for expand2max, 1 for expand2max_hier",
    )
    parser.add_argument(
        "--projection_intermediate_hidden_size",
        default=1024,
        type=int,
        help="span projection intermediate hidden size",
    )
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        logger.warning('args.no_cuda is {}'.format(args.no_cuda))
        logger.warning('test cuda device count {}'.format(torch.cuda.device_count()))
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_dev, best_test_run = train(args, train_dataset, model, tokenizer)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            if dev_results['f1'] > best_dev:
                ...
                model.to(args.device)
            test_results = evaluate(args, args.data_path, model, tokenizer, split="test")
            best_test = test_results['f1']
            ...
            logger.warning(f"EVAL best test at {args.data_path} -> metric is: {best_test_run}. ")
            best_test, best_dev = Decimal(best_test * 100).quantize(Decimal("0.01"),rounding="ROUND_HALF_UP"), Decimal(
                best_dev * 100).quantize(Decimal("0.01"), rounding="ROUND_HALF_UP"),
            data_name = args.data_path.split('/')[-1].lower()
            with open('result_' + data_name + '.txt', 'a') as writer:
                save_string = "\t".join(
                    [args.model_name_or_path, args.data_path, str(args.learning_rate), str(args.num_train_epochs),
                     str(args.expand_rate), str('ate is {}'.format(args.ate)),
                     str(args.per_gpu_train_batch_size), str(best_dev), str(best_test), str(best_test_run)])
